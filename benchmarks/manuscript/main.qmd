...
abstract: |
  ...
  **Results:** Schema validity was 112/112 (100%). Agent–baseline agreement was 80.4% (90/112). The agent never overrode the baseline when OncoKB was the sole evidence source (0/25 cases), preserving rule-based safety in the high-trust regime. The agent overrode 1/16 same-tumor concordant Tier I A cases (BCOR ITD in renal-cell-carcinoma, a histology-resolution disagreement) and 2/3 cross-tumor Tier I A cases (correctly downgrading RET M918T in NSCLC from inappropriate cross-tumor Tier I A to Tier II Level C with explicit `tumor_specificity_mismatch` flag). The dominant disagreement (20/112) was a documented Tier III VUS ↔ Tier II Level D rule-interpretation boundary for oncogenic variants without therapeutic levels. Run-to-run consistency was perfect: tier-call Fleiss kappa = 1.000 (10/10 unanimous across 3 runs).

  **Conclusions:** LLM-based evidence reconciliation provides reliable categorical clinical-tier output with reproducibility matching rule-based systems while detecting tumor-specificity discordances the deterministic rule misses. The benchmark and prompts are released as a reproducible Claude Code subagent + R orchestration framework.
...
**Metrics.** We measured: (a) **schema validity** — did the agent output parse and conform to the JSON schema; (b) **agreement with baseline** — did the agent's `unified_amp_tier` equal the deterministic baseline tier; (c) **stratified safety** — agreement broken down by whether the case's CiVIC evidence matched the queried tumor type; (d) **run-to-run consistency** — Fleiss kappa on tier-call across 3 independent runs of a 10-case subset spanning concordant, boundary, and tumor-specificity scenarios.
